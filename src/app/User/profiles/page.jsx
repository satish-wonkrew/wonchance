...
import { useUser } from "@/hooks/useUser"; // Adjust the import path as needed
import WorkDetails from "@/components/Profile/WorkDetails";
import Gallery from "@/components/Profile/Gallery";
import Link from "next/link"; // Import Link for navigating to update page
...
const ProfileCard = () => {
  ...
  const calculateAge = (dob) => {
    ...
    return Math.abs(ageDate.getUTCFullYear() - 1970); // Subtract 1970 to get the correct age
  };
  // Date of birth based culculete age
  ...
  const photosWithBaseUrl =
    user?.galleryDetails?.photos?.map((photo) => {
      return `${IMAGE_API_END_POINT}${photo.replace(/\\/g, "/")}`; // Convert to valid URL format
    }) || [];
  return (
