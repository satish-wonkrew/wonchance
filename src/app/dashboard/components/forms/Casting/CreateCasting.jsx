...
import { fetchRolesByProject, fetchRolesByProjectId } from "@/redux/slices/roleSlice";
import { createCasting } from "@/redux/slices/castingSlice";
import { toast } from "sonner";
import { fetchProjectsByCompany } from "@/redux/slices/projectSlice";
import { useUser } from "@/hooks/useUser";

export function CreateCastingCall({ onClose }) {
  const dispatch = useDispatch();
  const { user, status } = useUser();
  const { roles, status: rolesStatus } = useSelector((state) => state.roles);
  const { status: castingStatus } = useSelector((state) => state.castings);
  ...
  // Fetch roles when a project is selected
  useEffect(() => {
    if (selectedProject) {
          ...
          <TabsContent value="casting-call">
            <Card className="my-4">
              <CardHeader>
                ...
                <CardDescription>
                  Manage your casting calls here. Click Save when youre done.
                </CardDescription>
              </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        className="w-full border rounded-md p-2"
                        disabled={!selectedProject} // Disable role selection if no project is selected
                      >
